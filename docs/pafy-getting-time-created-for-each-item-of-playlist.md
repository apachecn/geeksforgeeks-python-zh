...
> 原文:[https://www . geesforgeks . org/pafy-为播放列表中的每个项目创建获取时间/](https://www.geeksforgeeks.org/pafy-getting-time-created-for-each-item-of-playlist/)

在本文中，我们将看到如何在 pafy 中为播放列表的每个项目获取时间创造值。Pafy 是一个 python 库，用于下载 YouTube 内容和检索元数据。Pafy 对象是包含给定视频所有信息的对象。YouTube 中的播放列表是按顺序播放的视频列表或组，一个视频接一个视频。
...
> 6。对于元数据，使用“time_created”键，这将返回给定项目的时间创建值
