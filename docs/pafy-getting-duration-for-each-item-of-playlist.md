...
> 原文:[https://www . geeksforgeeks . org/pafy-获取播放列表中每个项目的持续时间/](https://www.geeksforgeeks.org/pafy-getting-duration-for-each-item-of-playlist/)

在这篇文章中，我们将看到如何在 pafy 中获得播放列表的每个项目的持续时间。Pafy 是一个 python 库，用于下载 YouTube 内容和检索元数据。Pafy 对象是包含给定视频所有信息的对象。YouTube 中的播放列表是按顺序播放的视频列表或组，一个视频接一个视频。持续时间基本上是给定项目的时间，即给定项目有多长。以小时:分钟:秒为单位。
